...
async def process_webhook(
    ...
    Performs no I/O and never suspends, because Netatmo requires a webhook
    endpoint to answer immediately and deactivates one that does not. Do not add
    an `await` here: it would put network latency, including the 429 backoff in
    `auth.py`, on the response path.
...
def _process_state(
    ...
    touched: list[str] = []
    if event_type in (EVENT_TYPE_SET_POINT, EVENT_TYPE_CANCEL_SET_POINT):
