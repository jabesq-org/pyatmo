...
def test_post_request_json(auth, requests_mock):
    ...
    requests_mock.post(
        pyatmo.const._BASE_URL,
        json={"a": "b"},
        headers={"content-type": "application/json"},
    )
    resp = auth.post_request(pyatmo.const._BASE_URL, None).json()
    assert resp == {"a": "b"}
...
def test_post_request_binary(auth, requests_mock):
    ...
    resp = auth.post_request(pyatmo.const._BASE_URL, None).content
...
def test_post_request_fail(auth, requests_mock, test_input, expected):
    ...
    requests_mock.post(pyatmo.const._BASE_URL, status_code=test_input)
